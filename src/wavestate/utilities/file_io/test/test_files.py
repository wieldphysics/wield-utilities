...
from transient import file_io
from transient.file_io import (
    matlab_io,
)
...
try:
    from transient_test_data import matlab as matlab_test_data
except ImportError:
    module_import_skip = True
else:
    module_import_skip = False

@pytest.mark.skipif(module_import_skip, reason = 'cannot import transient_test_data')
def test_mat():
...
def test_mat_struct_RT():
    ...
    matlab_io.write_matlab(fpath, fdict)
